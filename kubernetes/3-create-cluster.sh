...
##########################################################################################################################
eksctl create iamserviceaccount --cluster="$CLUSTER_NAME" --namespace=kube-system --name=aws-load-balancer-controller --attach-policy-arn=arn:aws:iam::876764840442:policy/AWSLoadBalancerControllerIAMPolicy --approve
...
##########################################################################################################################
kubectl apply -f blue-useast1-ingress_class.yaml
sleep 20s
kubectl apply -f blue-useast1-ingress.yaml