...
router.post("/createQuiz", async (req, res) => {
  ...
  async function uploadQuizToS3(quizData, bucketName, fileName) {
    const params = {
      Bucket: "ak-costs",    // Name of the S3 bucket
      Key: `quiz-${title}_${newQuiz._id}1.json`,         // File name under which the data will be saved
      Body: JSON.stringify(quizData),  // Stringified quiz data
    ...
    try {
      ...
      return data; // Contains information about the upload
    } catch (err) {
      console.error('Error uploading to S3:', err);
      throw err; // Rethrow for error handling in the route
    }
  }

  async function triggerLambda(questions) {
    const command = new InvokeCommand({
      FunctionName: process.env.LAMBDA_FUNCTION_NAME,  // Name of the Lambda function
      Payload: Buffer.from(JSON.stringify({ questions })),  // Payload sent to Lambda
    });
  
    try {
      const data = await lambdaClient.send(command);
      console.log('Lambda function triggered successfully:', data);
    } catch (err) {
      console.error(err);
      throw err; // Rethrow to handle it in the route handler
    }
  }

  const fileName = `quiz-${title}_${newQuiz._id}.json`;
  await uploadQuizToS3({ title, questions }, 'fp-extract-quiz-data', fileName);
  console.log("s3");

  // Trigger Lambda function after saving the quiz
  await triggerLambda(questions);
  console.log("lambda");
